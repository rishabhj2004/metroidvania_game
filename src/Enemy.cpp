...
bool Enemy::updateDeath(float dt)
{
    ...
    if (deathAnimation.isFinished())
    {
        ...
        if (deathTimer >=
            deathHoldTime + deathDissolveTime)
        {
...
bool Enemy::updateKnockback(float dt)
{
    ...
    {
        shape.move(
            direction * std::abs(knockbackDistance),
            0.f
        );
    ...
    knockbackSpeed -= knockbackDeceleration * dt;
...
void Enemy::takeDamage(int damage)
{
    ...
    if (health <= 0)
    {
        ...
        velocity.x = 0.f;
        velocity.y = -200.f;
        knockbackSpeed = 350.f;
        knockbackDeceleration = 600.f;
        knockbackDistance *= 15.f;
